...
**[在线 Demo](https://skillnet-demo.onrender.com)** · **[链上界面](https://skillnet-ten.vercel.app)** · **[English](README.md)**
...
- **真正能结清的微支付** —— 单次调用费用低于一美分,没有任何链能逐笔结算(光 gas 下限就是支付额的 2.5 倍以上)。调用在链下以 **EIP-712 累计凭证**记账,批量上链结算:**200 笔亚分调用 → 2 笔交易**,守恒由合约强制。
- **无人能下架的注册表** —— 技能身份、规范化 `contentHash`、创作者 Ed25519 签名,全部锚定在 **Hedera HCS-26**;读取时验证,无法伪造、无法移除。
...
# Demo 服务器 —— 19 个 node:test 测试(支付语义、鉴权、持久化、防漂移)
...
以 [issues](https://github.com/GaryGAO2003/skillnet/issues) 追踪:结算网关闭环(存款 → 调用 → 凭证 → 链上批量结算 → 提现,端到端)、创作者独立注册身份 + 可搜索注册表界面、真实 x402/USDC 支付轨道。
